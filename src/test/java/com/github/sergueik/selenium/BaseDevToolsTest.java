...
//cannot access org.openqa.selenium.chrome.ChromeDriver

//class file has wrong version 55.0, should be 52
...
public class BaseDevToolsTest {
	...
	public static void setUp() throws Exception {
		...
		// see also: https://www.baeldung.com/junit-conditional-assume

		try {
			chromeDevTools.createSession();

		} catch (DevToolsException e) {
			// you are using a no-op implementation of the CDP.
			...
			// Please be sure to include an implementation on the classpath, possibly by adding a new (maven) dependency of `
			// org.seleniumhq.selenium:selenium-devtools-vNN:NNN` where `NN` matches the major version of the browser you're using
			//
	...
	public boolean pingHost(String host, int port, int timeout) {
		...
		try (Socket socket = new Socket()) {
			...
			System.err.println(String.format("Trying to connect to host %s port %d", host, port));
