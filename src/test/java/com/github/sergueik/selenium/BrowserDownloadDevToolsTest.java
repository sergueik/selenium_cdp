...
import org.openqa.selenium.devtools.v119.browser.model.DownloadProgress;
import org.openqa.selenium.devtools.v119.browser.model.DownloadWillBegin;
...
public class BrowserDownloadDevToolsTest extends BaseDevToolsTest {
	...
	public void beforeTest() throws Exception {
		new File(Paths.get(tempDownloadDirectory).resolve(filename).toAbsolutePath()
				.toString()).delete();
	}
		...
		try {
			...
			chromeDevTools.addListener(Browser.downloadProgress(), o -> {
				...
				if (o.getTotalBytes().doubleValue()  > 0f) 
					System.err.println(String.format("%8.2f %% downloaded.", Math.ceil(100.0 * o.getReceivedBytes().doubleValue() / o.getTotalBytes().doubleValue())));
